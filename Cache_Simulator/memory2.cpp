...
void MemoryClass::WriteByte(unsigned int address, bitset<8> data){

  if (address >= 0 && address < 0x10000) {
    textSectionstart.WriteByte(address,data);

  } else if (address >= 0x10000 && address < 0x20000) {
    dataSectionstart.WriteByte(address-0x10000,data);
    
  } else if (address >= 0x20000 && address < 0x30000) {
     dataSection2.WriteByte(address-0x20000,data);

  } else if (address >= 0x30000 && address < 0x40000) {
    dataSection3.WriteByte(address-0x30000,data);

  } else if (address >= 0x40000 && address < 0x50000) {
    stackSection.WriteByte(0x50000-address-1, data);
  }

}

bitset<8> MemoryClass::ReadByte(unsigned int address){
  bitset<8> a=0;
  if (address >= 0 && address < 0x10000) {
    a=textSectionstart.ReadByte(address);

  } else if (address >= 0x10000 && address < 0x20000) {
    a=dataSectionstart.ReadByte(address-0x10000);
    
  } else if (address >= 0x20000 && address < 0x30000) {
     a=dataSection2.ReadByte(address-0x20000);

  } else if (address >= 0x30000 && address < 0x40000) {
    a=dataSection3.ReadByte(address-0x30000);

  } else if (address >= 0x40000 && address < 0x50000) {
    a=stackSection.ReadByte(0x50000-address-1);
  }
