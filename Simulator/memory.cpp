...
// address is index
void textSectionClass::writeData(int n, unsigned int address, bitset<64> value) {
  if (mem.size() - top <= n) {
    mem.resize(top + 4 * S, 0);
  }
...
// address is index
void dataSectionClass::writeData(int n, unsigned int address, bitset<64> value) {
  if (mem.size() - top <= n) {
    mem.resize(top + 4 * S, 0);
  }
  while (address + n - 1 >= mem.size()) {
...
// adress is index
void stackSectionClass::writeData(int n, unsigned int address, bitset<64> value) {
  if (mem.size() - top <= n) {
    mem.resize(top + 4 * S, 0);
  }
...
void textSectionClass:: printMemmory() const{
  for(int i=0; i<top;i++){
    std::cout<<"Memory[0x"<<hex<<i<<"] = 0x"<<hex<<mem[i].to_ulong()<<endl;
  }
...
void dataSectionClass:: printMemmory(unsigned int start_address) const{
  for(int i=0; i<top;i++){
    std::cout<<"Memory[0x"<<hex<<i+start_address<<"] = 0x"<<hex<<mem[i].to_ulong()<<endl;
  }

  cout<<"From 0x"<<hex<<top+start_address<<" to 0x"<<hex<<(0x10000+start_address) <<" contains 0x00"<<endl;

}

void stackSectionClass:: printMemmory() const{
  cout<<"From 0x"<<hex<<0x40000<<" to 0x"<<hex<<(0x50000-top-1) <<" contains 0x00"<<endl;
  for(int i=0; i<top;i++){
    std::cout<<"Memory[0x"<<hex<<(0x50000-i-1)<<"] = 0x"<<hex<<mem[i].to_ulong()<<endl;
  }
